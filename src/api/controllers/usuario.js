...
export const recuperacao = async (req, res) => {
    ...
    db.query(consulta, [email], async (erro, usuario) => {
        if(erro){
            ...
            if (!usuario[0]) {
                return res.status(400).json({ msg: "Usuário não encontrado ou campo não preenchido."})
            } else {
                mailer.sendMail(
                    {
                        to: email,
                ...
                db.query(atualizacao, [token, expiracao, email], async () => {
                    return res.status(201).json({ msg: "E-mail de recuperação enviado." })
                })
            }
        }
    ...
    try{
        ...
        db.query(consulta, [email, token], async (erro, usuario) => {
            if(erro){
                ...
                if (!usuario[0]) {
                    res.status(400).json({msg: "Usuário não encontrado ou campo não preenchido."})
                } else {
