...
export const addUsuario = async (req, res) => {
    ...
    db.query(insert, [nome_usuario, email, senha, telefone],(erro, results) => {
            if (erro)
...
export const recuperacao = async (req, res) => {
    ...
    db.query(consulta, [email], async (erro, usuario) => {
        try {
                ...
                mailer.sendMail(
                    {
                        ...
                        html:
                            "<h1>Esqueceu sua senha?</h1><p>Aparentemente você deseja trocar sua senha no SeuCERT.</p> <p>Caso você de fato queira redefinir sua senha, utilize o token:</p> <h2>" +
                            token +
                            "</h2>",
                    },
                    (erro) => {
            ...
            res.status(500).json({
                erro: "Houve um erro na recuperação de senha!",
            })
        }
    }),
            ...
            if (!usuario) {
                ...
                if (token != usuario.token) {
                    ...
                    if (usuario.expiracao_token > agora) {
                        db.query("UPDATE usuario SET senha = ?, token = NULL, expiracao_token = NULL WHERE usuario.email=?", [senha, email], async (erro) => {
                                if (erro)
