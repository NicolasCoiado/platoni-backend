...
export const editCertificado = async (req, res) => {
  ...
  const update = "UPDATE certificados SET nome_certificado = ?, emissor = ?, descricao = ?, url = ?, id_url = ? WHERE id_certificado = ?"
  const update2 = "UPDATE certificados SET nome_certificado = ?, emissor = ?, descricao = ? WHERE id_certificado = ?"
  ...
  try {
    if (req.file){
      db.query(select, [id_certificado],(erro, resultado) => {
        if (erro){
            return res.status(400).json({msg: "Erro ao consultar certificado.", erro})
        }else{
      ...
      db.query(update2, [nome_certificado, emissor, descricao, id_certificado],(erro) => {
        if (erro)
            return res.status(400).json({msg: "Erro ao atualizar certificado.", erro})
        else   
            return res.status(201).json({msg: "Certificado atualizado com sucesso."})
  ...
  try {
    db.query(select, [id_certificado],(erro, resultado) => {
      if (erro){
        ...
        cloudinary.uploader.destroy(id_url, function (err){
          if(err){
            return res.status(500).json({msg: "Erro ao excluir imagem do certificado.", err})
          }else{
